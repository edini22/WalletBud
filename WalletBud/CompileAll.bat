...
"%_JAVACCMD%" -encoding UTF-8 -d %_CLASSES% -classpath %_CLASSPATH% %_SOURCEPATH%\java\WB\walletbud\*.java %_SOURCEPATH%\java\ormsamples\*.java
if not exist "%_CLASSES%\ormmapping" md %_CLASSES%\ormmapping
if not %_SOURCEPATH% == %_CLASSES% xcopy %_SOURCEPATH%\resources\ormmapping %_CLASSES%\ormmapping /s
