...
public class Transacao {
    ...
    @Secured
    @Produces(MediaType.APPLICATION_JSON)
    public Response listGastosSemana(@HeaderParam(HttpHeaders.AUTHORIZATION) String authorizationHeader) throws PersistentException {
        String token = authorizationHeader.substring("Bearer ".length()).trim();
        String email = JWTUtil.getEmailFromToken(token);
        ...
        try {
            ...
            JsonObject gastos = gerirTransacaoPartilhada.getBalanco(session, email);
