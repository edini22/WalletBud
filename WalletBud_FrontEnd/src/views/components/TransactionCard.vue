    ...
    <div class="card h-100">
        <div class="card-header pb-0 px-3">
            <div class="d-flex flex-wrap">
                <div class="me-2" style="width: 150px;">
                    <!-- time period-->
                    <MaterialDropdown :on-click='changeFilter' :options='time_periods'
          ...
          <MaterialFilter @change="handleFilterChange" />
                ...
                <div class="me-2">
                    <!-- search movements -->
                    <div class="input-group input-group-outline" :class="{ 'is-focused': isFocused }">
                        <label class="form-label">{{ $t('Procurar movimentos') }}</label>
                        <input class="form-control form-control-default" id="search" style='width: 170px;'
                            v-model="searchQuery" @focus="handleFocus" @blur="handleBlur" />
                    </div>
                </div>
        ...
        <div class="card-body pt-2 p-3 pb-1">
            <ul class="list-group">
                <li class="list-group-item border-0 d-flex justify-content-between ps-0 mb-2 border-radius-lg"
                    data-bs-toggle='modal' data-bs-target='#detailsModal' v-for="t in paginatedTransactions" :key="t.id"
                    :tID="t.id" @click="openTransactionDetails(t)">
                    <div class="d-flex align-items-center">
                        <material-button style="cursor:auto;" v-if="t.tipo == 'despesa'" color="danger"
                            variant="outline" size="sm"
                            class="btn-icon-only btn-rounded mb-0 me-3 d-flex align-items-center justify-content-center">
                            <i class="fas fa-arrow-down" aria-hidden="true"></i>
                        </material-button>
                        <material-button style="cursor:auto;" v-if="t.tipo == 'receita'" color="success"
                            variant="outline" size="sm"
                            class="btn-icon-only btn-rounded mb-0 me-3 d-flex align-items-center justify-content-center">
        ...
        <div class="card-footer p-3 size" v-if="totalPages > 1">
            <material-pagination color="info" size="sm">
                ...
                <material-pagination-item v-for="page in totalPages" :key="page" :label="page"
                    :active="page === currentPage" @click="goToPage(page)" />
...
export default {
    ...
    setup() {
        ...
        // Watch activeFilter to load transactions based on the filter
